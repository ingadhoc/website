...
class WebsitePromotion(models.Model):
    ...
    public_category_id = fields.Many2one("product.public.category", required=True)
    template_ids = fields.Many2many(
        "product.template", "website_promotion_product_rel", "website_promotion_id", "product_id", "Product Template"
    )
    website_style_id = fields.Many2one("product.ribbon", "Website Ribbon")
    state = fields.Selection(
        [("draft", "Draft"), ("confirm", "Confirm"), ("finished", "Finished")],
        default="draft",
    )
    ...
    def confirm(self):
        for rec in self:
            ...
            if rec.website_style_id:
                rec.template_ids.write({"website_ribbon_id": rec.website_style_id.id})
            rec.template_ids.write({"public_categ_ids": [(4, rec.public_category_id.id)]})
            vals_list = [
                {
                    "name": rec.name,
    ...
    def finished(self):
        for rec in self:
            ...
            if items:
                items.unlink()
            if rec.website_style_id:
                rec.template_ids.write({"website_ribbon_id": False})
            rec.template_ids.write({"public_categ_ids": [(3, rec.public_category_id.id)]})
